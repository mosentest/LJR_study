...
<!--           <button class="btn btn-success btn-sm" id="add"><i class="icon-plus-sign align-top bigger-125"></i>添加</button> -->
              ...
              <table id="table" class="table table-striped table-bordered table-hover dataTable" aria-describedby="sample-table-2_info"> 
               <thead> 
                <tr role="row"> 
                 ...
                 <th role="columnheader"  rowspan="1" colspan="1" style="width: 130px;" >更新日期</th>
                 <th role="columnheader"  rowspan="1" colspan="1" style="width: 130px;" >次数</th>
	...
	function gotoPage(pageIndex, cond) {
		...
		$('#tb').html("");
		$.ajax({
			url : "userwrongdiscipline/list.html",
			type : 'get',
				...
				if(msg.page.totalElement == 0){
					...
					$.each(msg.page.content, function(i, item) {
			              $('#tb').append( "<tr>"
			            		  ...
			            		  +"<td >"+getSmpFormatDateByLong(item.updateDate,"yyyy-MM-dd")+"</td> "
			            		  +"<td >"+item.count+"</td> "
			            		  +"<td >"+"<div class='visible-md visible-lg hidden-sm hidden-xs action-buttons' id='buttontools'>"
// 			            		  				+"<a class='green' href='userWrongDiscipline/showOne.html?id="+item.id+"' > <i class='icon-pencil bigger-130'></i> </a>"
// 			            		  				+"<a class='red' href='userWrongDiscipline/delete?id="+item.id+"' > <i class='icon-trash bigger-130'></i> </a>"
			            		  				+"</td> "+"</tr>");
			            });
